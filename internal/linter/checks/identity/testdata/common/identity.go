...
func (unidentified) Configure(c dogma.ApplicationConfigurer) {} // EXPECT(error): Configure() must call c.Identity()
...
func (someBranches1) Configure(c dogma.ApplicationConfigurer) {
	if someCondition { // EXPECT(error): `this control-flow statement causes c.Identity\(\) to remain uncalled on some execution paths`
...
func (duplicate1) Configure(c dogma.ApplicationConfigurer) {
	...
	c.Identity("name", "05d1e6ec-41a0-43b4-a866-4b5cce3bf2fd") // EXPECT(error): `c.Identity\(\) must be called exactly once`
	c.Identity("name", "05d1e6ec-41a0-43b4-a866-4b5cce3bf2fd") // EXPECT(error): `c.Identity\(\) must be called exactly once`
}
...
func (duplicate2) Configure(c dogma.ApplicationConfigurer) {
	...
	c.Identity("name", "c454bbac-1a93-49d0-8d8c-a4671e3e3025") // EXPECT(error): `c.Identity\(\) has already been called on at least one execution path`
}
