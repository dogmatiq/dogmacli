...
func (invalidValues) Configure(c dogma.ApplicationConfigurer) {
	c.Identity(
		"", // EXPECT(error): `invalid name "", names must be non-empty, printable UTF-8 strings with no whitespace`
		"", // EXPECT(error): `invalid key "", keys must be non-empty, printable UTF-8 strings with no whitespace`
	)
}
...
func (nonUUIDKey) Configure(c dogma.ApplicationConfigurer) {
	c.Identity(
		"name",
		"key", // EXPECT(error): `identity keys should be UUIDs \(invalid UUID length: 3\)`
	)
}
...
func (alternateUUIDKey) Configure(c dogma.ApplicationConfigurer) {
	c.Identity(
		"name",
		"{bc5c4138-9ead-4d17-a425-88d0e4cb3059}", // EXPECT(error): `identity key UUIDs should use RFC 4122 hex notation \(xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx\)`
	)
}
